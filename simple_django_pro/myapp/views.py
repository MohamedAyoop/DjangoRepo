...
def blogger_signup(req,usr):
    if 'submit' in req.POST:
        ...
        if username:
            ...
            user.role = 'b'
            user.save()
    return render(req, 'myapp/blogger_signup.html',{'user':usr})
def signin(req):
    if 'log' in req.POST:
...
def user_page(req, usr):
    ...
    return render(req, 'myapp/user_page.html', {'Username':usr, 'blogs':blg})
def view_page(req,usr):
    blg = Blogs.objects.all()
    user = User.objects.filter(usr_name = usr).first()
    return render(req, 'myapp/view_page.html',{'Username':user.usr_name, 'blogs':blg, 'status':user.role})